...
from Eval.Metrics import oneshot_accuracy
keras = tf.keras
L = keras.layers
...
def omniglot_experiment(hparams: dict=None, settings: dict=None, verbose=1):
    ...
    for i in range(n_rounds):
        images, labels = omniglot_loader.get_train_batch(batch_size, support_size)
        loss = model.train_on_batch(images, labels)
        train_records.append(loss)
        if verbose == 1:
            print("Iteration:", i, " Training loss & metric:", loss)
        if i % 100 == 0:
            eval_imgs, eval_labels = omniglot_loader.get_test_batch(test_batch_size, support_size)
            pred_labels = model.predict(eval_imgs)
            # print(pred_labels)
            acc = model.evaluate(eval_imgs, eval_labels, verbose=0)
            oneshot_acc = oneshot_accuracy(pred_labels, support_size)
            test_records.append((i, acc, oneshot_acc))
            if verbose == 1:
                print("Test loss & metric:", acc)
                print("Oneshot accuracy {:.4f}".format(oneshot_acc))
