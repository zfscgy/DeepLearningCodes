...
class OmniglotLoader:
    ...
    Attributes:
        ...
        use_augmentation: boolean that allows us to select if data augmentation is 
            used or not
        image_augmenter: instance of class ImageAugmentor that augments the images
    ...
    def __init__(self, use_augmentation, batch_size):
        ...
        if self.use_augmentation:
            self.image_augmenter = self.create_augmentor()
        else:
    ...
    def get_train_batch(self):
        ...
        ** Only return characters from one alphabet in one batch**
        ** 23 alphabets used for training **
        ...
        Returns:
            pairs_of_images: pairs of images for the current batch
                shape: [2, W, H, C] here C = 1
            labels: correspondent labels -1 for same class, 0 for different classes

        """

        # Only use one alphabet
        ...
        if self.use_augmentation:
            images = self.image_augmenter.get_random_transform(images)
    ...
    def get_test_batch(self, support_set_size, is_validation, number_of_tasks_per_alphabet):
        # Set some variables that depend on dataset
        if is_validation:
            alphabets = self._validation_alphabets
            print('\nMaking batch on validation alphabets:')
        else:
            alphabets = self._evaluation_alphabets
            print('\nMaking batch on evaluation alphabets:')

        image_batch = [[], []]
        label_batch = []
        for alphabet in alphabets:
            for _ in range(number_of_tasks_per_alphabet):
                images, labels = self.get_one_shot_batch(
                    support_set_size, is_validation=is_validation)
                image_batch[0].append(images[0])
                image_batch[1].append(images[1])
        ...
        images = [np.concatenate(image_batch[0], axis=0), np.concatenate(image_batch[1], axis=0)]
