...
class GRU4Rec:
    def __init__(self, item_size, embedding_dim, seq_len,
        ...
        self.gru_rnn = L.RNN(self.gru_cell, return_sequences=False)
        self.gru_out = self.gru_rnn(self.embedding_seq)  # [batch_size, gru_units]
        self.gru_dropout_layer = L.Dropout(dropout_rate)
        self.dropout_out = self.gru_dropout_layer(self.gru_out)
        self.out_dense_layer = L.Dense(item_size, activation=A.linear)
        self.out_ratings = self.out_dense_layer(self.dropout_out)
